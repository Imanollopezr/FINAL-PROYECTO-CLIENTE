  ...
  const procederAlPago = async () => {
    try {
      ...
      try {
        ...
        const profileResp = await fetch(buildApiUrl(API_ENDPOINTS.AUTH.PROFILE), {
          method: 'GET',
          headers,
          credentials: 'include'
        });
        if (profileResp.ok) {
          const raw = await profileResp.text();
          let data = {};
          try { data = raw ? JSON.parse(raw) : {}; } catch { data = {}; }
          const u = data?.data?.usuario || data?.data || data?.Usuario || data?.usuario || {};
          const n = u?.nombres ?? u?.nombre ?? u?.Nombre ?? '';
          const a = u?.apellidos ?? u?.apellido ?? u?.Apellido ?? u?.Apellidos ?? '';
          if (String(n).trim()) nombreDefault = String(n).trim();
          if (String(a).trim()) apellidoDefault = String(a).trim();
        }
      } catch { }
      let me = null;
      ...
      if (me) {
        const candNombre = me?.nombres ?? me?.nombre ?? me?.Nombre ?? '';
        const candApellido = me?.apellidos ?? me?.apellido ?? me?.Apellido ?? me?.Apellidos ?? '';
        if (!String(nombreDefault).trim() && String(candNombre).trim()) {
          nombreDefault = String(candNombre).trim();
        }
        if (!String(apellidoDefault).trim() && String(candApellido).trim()) {
          apellidoDefault = String(candApellido).trim();
        }
      }
      const formHtml = `
        <div class="checkout-form">
          <div class="cliente-resumen">
            Nombre: ${String(nombreDefault || '').trim() || 'No disponible'} Apellido: ${String(apellidoDefault || '').trim() || 'No disponible'}
          </div>
          <div class="form-row">
          ...
          const lookupClientePorDocumento = async () => {
            ...
            try {
              ...
              if (exact) {
                ...
                if (apellidoInput && !apellidoInput.readOnly) {
